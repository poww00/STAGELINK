...
const FindIdPage = () => {
  ...
  const handleFindId = async (e) => {
    ...
    e.preventDefault();
    try {
      const res = await axios.post("http://localhost:8080/api/member/find-id", {
        name,
        userEmail,
      });
      setFoundId(res.data);
  ...
  <>
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center px-4">
      <div className="bg-white w-full max-w-lg min-h-[400px] px-10 py-16 rounded-2xl shadow-lg flex flex-col justify-center">
        ...
        <form onSubmit={handleFindId} className="mx-auto w-full max-w-sm">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-2xl p-8 shadow-lg w-[90%] max-w-md text-center">
            <h2 className="text-lg font-bold text-gray-800 mb-4">아이디 찾기 결과</h2>
            <p className="text-purple-600 text-2xl font-semibold mb-6">
              {foundId}
            </p>
            <button
